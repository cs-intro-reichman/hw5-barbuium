...
public class Scrabble {
	...
	public static String createHand() {
		...
		int randomIndexForA = (int) (Math.random() * (HAND_SIZE - 1));
		hand.insert(randomIndexForA, 'a');

		int randomIndexForE = (int) (Math.random() * HAND_SIZE);
		while (randomIndexForE == randomIndexForA) {
			randomIndexForE = (int) (Math.random() * HAND_SIZE);
		}
		hand.insert(randomIndexForE, 'e');
	...
	public static void playHand(String hand) {
		...
		while (hand.length() > 0) {
			...
			if (input.equals(".")) {
				break;
		} if (!canFormWordFromHand(input, hand)) {
			System.out.println("You can't form that word with the current hand. Please try again.");
			continue;
		}
		...
		hand = removeLettersFromHand(input, hand);
